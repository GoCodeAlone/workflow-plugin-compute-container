...
func (i ManagedRuntimeBundleInstaller) Install(ctx context.Context, request ManagedRuntimeInstallRequest) (ManagedRuntimeInstallResult, error) {
	...
	source := i.source()
	artifact, checksum, signature, err := i.fetchPinnedObjects(ctx, source, bundle)
	if err != nil {
		return ManagedRuntimeInstallResult{}, err
	}
	...
	if len(signature) == 0 {
		return ManagedRuntimeInstallResult{}, errors.New("managed runtime signature object is empty")
	}
...
func (i ManagedRuntimeBundleInstaller) Reinstall(ctx context.Context, request ManagedRuntimeInstallRequest) (ManagedRuntimeReinstallResult, error) {
	uninstall, err := i.Uninstall(ctx, ManagedRuntimeUninstallRequest{BundleID: request.BundleID})
	if err != nil {
...
func (i ManagedRuntimeBundleInstaller) fetchPinnedObjects(ctx context.Context, source ManagedRuntimeBundleObjectSource, bundle core.ManagedRuntimeBundleDescriptor) ([]byte, []byte, []byte, error) {
	...
	return artifact, checksum, signature, nil
}
...
func fetchManagedRuntimePinnedObject(ctx context.Context, catalog ManagedRuntimeBundleCatalog, source ManagedRuntimeBundleObjectSource, bundle core.ManagedRuntimeBundleDescriptor, kind, name, wantDigest string) ([]byte, error) {
	...
	object, err := source.FetchManagedRuntimeBundleObject(ctx, ManagedRuntimeBundleObjectRequest{
		...
		Kind:     kind,
		Name:     name,
		URL:      managedRuntimeObjectURL(catalog.SourceBaseURL, name),
	})
	if err != nil {
...
func managedRuntimeBundleRoot(installRoot, bundleID string) (string, string, error) {
	...
	if err := managedRuntimeRejectSymlinkedInstallRoot(root); err != nil {
		return "", "", err
	}
...
func managedRuntimeRejectSymlinkedInstallRoot(root string) error {
	...
	if info.Mode()&os.ModeSymlink != 0 {
		return fmt.Errorf("managed runtime install root %q is a symlink", root)
	}
	return nil
}
